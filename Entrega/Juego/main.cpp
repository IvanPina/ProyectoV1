...
int main()
{
    ...
    nodo prueba2=nodo(90.f,120.f);
    //donde crear nodos??
    //clase mapa
    e.setdestino(&prueba);
    e.setdestino(&prueba2);
    e.setdestino(&prueba);
    ...
    while (window.isOpen())
    {
        ...
        */
        for(int i=0;i<enemigos.size();i++){
            ...
            window.draw(enemigos[i]->getpistola());
            window.draw(enemigos[i]->getbala());
            //std::cout << enemigos[i]->getbala().getPosition().x << std::endl;
            //std::cout << enemigos[i].getforma().getRadius() << std::endl;
